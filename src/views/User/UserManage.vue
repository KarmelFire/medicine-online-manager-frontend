  ...
  <div class="table-box">
    ...
    >
      ...
      <template #tableHeader>
        <el-button type="primary" :icon="Download" plain @click="downloadFile" v-hasPermi="['sys:user:export']"> 导出⽤户 </el-button>
      </template>
      <!-- 表格操作 -->
...
const columns: ColumnProps<UserType>[] = [
  ...
  },
  {
    ...
    label: '性别',
    width: 100,
    enum: () => dictConfigList('gender'), // 使用字典展示性别
    search: {
      el: 'select',
      props: {
        filterable: true,
    ...
    fieldNames: { label: 'title', value: 'value' }, // 配置字典字段
    render: (scope) => {
...
const actionUser = async (title: string, row: UserType) => {
  ElMessageBox.confirm(`确认${title}【${row.nickname}】用户?`, '温馨提示', { type: 'warning' }).then(async () => {
    try {
      await UserApi.freezeUser(row.pkId) // 调用冻结/启用用户的接口
      ElMessage.success(`${title}用户成功`) // 成功提示
      // 重新获取数据
      proTable.value.getTableList()
    } catch (error) {
      ElMessage.error(`${title}用户失败`) // 失败提示
    }
  })
}
